...
import { GetProduct, GetProductByCategory, AddProduct, DeleteProduct, GetProductById, GetBestSeller, GetProductsByType, Get3DObjectProductDetail,GetListSize, GetListColor } from './factory'
...
export function* GetProductsByTypeSaga() {
    yield takeEvery(actions.GET_LIST_PRODUCT_BY_TYPE, function* (payload) {
        const { data, onSuccess, onError } = payload
        try {
            const response = yield call(() => GetProductsByType.requestGetProductsByType(data))
            if (response?.data?.code === 'ok') {
                onSuccess && onSuccess(response);
            } else {
                onError && onError("onEnrror")
...
export function* Get3DObejectDetailProd() {
    yield takeEvery(actions.GET_OBJECT3D_PRODUCT_DETAIL, function* (payload) {
        const { data, onSuccess, onError } = payload
        try {
            const response = yield call(() => Get3DObjectProductDetail.requestGet3DObjectProductDetail(data))
            if (response?.data?.code === 'ok') {
                onSuccess && onSuccess(response);
            } else {
                onError && onError("onEnrror")
...
export function* getListSize() {
    yield takeEvery(actions.GET_LIST_SIZE, function* (payload) {
        const { data, onSuccess, onError } = payload
        try {
            const response = yield call(() => GetListSize.requestGetListSize())
            if (response?.status === 200) {
                onSuccess && onSuccess(response);
            } else {
                onError && onError(response?.response?.status)
...
export function* getListColor() {
    yield takeEvery(actions.GET_LIST_COLOR, function* (payload) {
        const { data, onSuccess, onError } = payload
        try {
            const response = yield call(() => GetListColor.requestGetListColor())
            if (response?.status === 200) {
                onSuccess && onSuccess(response);
            } else {
                onError && onError(response?.response?.status)
...
export default function* getProduct() {
    yield all([
        ...
        fork(GetProductsByTypeSaga),
        fork(Get3DObejectDetailProd),
        fork(getListColor),
        fork(getListSize),
    ])
}
